...
async def test_prompt(
    req: TestPromptRequest, db: Session = Depends(get_db), admin=Depends(require_admin)
):
    if req.server_id:
        server = (
            db.query(OllamaServer)
            .filter(OllamaServer.id == req.server_id)
            .first()
        )
    else:
...
async def cluster_test(
    req: ClusterTestRequest, db: Session = Depends(get_db), admin=Depends(require_admin)
):
    servers = (
        db.query(OllamaServer)
        .filter(OllamaServer.enabled == True, OllamaServer.is_healthy == True)
        .all()
    )
    results = []
    for s in servers:
        has_model = any(
            m.model_name == req.model and m.available for m in s.models
        )
        if not has_model:
            continue
