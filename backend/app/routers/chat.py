...
from app.schemas.server import ChatCompletionRequest, ChatCompletionResponse, ChatCompletionChoice
...
async def stream_chat_response(client, payload: dict, server: OllamaServer, log: RequestLog, start_time: float):
    completion_text = ""
    prompt_tokens = 0
    completion_tokens = 0
    try:
            ...
            if line.strip():
                try:
                    ...
                    event = {
                        ...
                        "model": payload["model"],
                        "choices": [{"index": 0, "delta": {"content": data.get("message", {}).get("content", "")}, "finish_reason": None}]
                    }
...
async def chat_completions(
    ...
    if not server or not client:
        raise HTTPException(status_code=503, detail="No available Ollama servers for this model")
